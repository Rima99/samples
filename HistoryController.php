...
use AppBundle\Service\AccessControlManager;
...
class HistoryController extends Controller
{
    ...
    public function checkingUserAccess(Request $request)
    {
        ...
        $userRoleId = $this->getUser()->getUserRole()->getId();


        if ($userRoleId != ConstModel::company_boss && $userRoleId != ConstModel::loyal_role) {
            return $this->redirectToRoute('fos_user_security_logout');
        }
        if ($access->checkLoginAccess($user) == 0)
            return $this->redirectToRoute('web_confirmation');
        if($access->checkLoginAccess($user) == 3)
            return $this->redirectToRoute('phone_main_confirmation');
    }
    ...
    public function findSalesHistory(Request $request,$branchId,$_locale="am")
    {
        ...
        $redirect = $this->checkingUserAccess($request);
        if($redirect!=false)
            return $redirect;
    ...
    public function findAccumulatesHistory(Request $request,$branchId,$_locale="am"){
        ...
        $documentRepository = $this->getDoctrine()->getManager()->getRepository('AppBundle:Document');
        $docQuery = $documentRepository->createQueryBuilder('d')
            ->select('u.username,d.documentDate, d.bonusPoints, b.branchTitle')
    ...
    public function findCardHistoryAccumulates(Request $request,$id,$_locale="am"){
        ...
        if($customer) {
            if (null == $customer->getUser()) {
                    ...
                    $docs = $em->createQuery(
                        'SELECT d.amount, d.bonusPoints,d.documentDate, b.branchTitle
                            FROM AppBundle:Document d 
                            JOIN AppBundle:Account a WITH d.accountTo = a 
                            JOIN AppBundle:Account acc WITH d.accountFrom = acc
                            
                            LEFT JOIN AppBundle:User u WITH a.user = u 
                            JOIN AppBundle:User us WITH acc.user = us
                            
                            JOIN AppBundle:BranchUser bu WITH bu.user = us
                            JOIN AppBundle:Branch b WITH bu.branch = b
                        ...
                        ->setParameter('user', $customer->getUser())
                ...
                $docs = $em->createQuery(
                    'SELECT d.amount, d.bonusPoints,d.documentDate, b.branchTitle
                            ...
                            WHERE u=:user_ AND (t.id =:typeId AND d.documentDate >= :now )
                            ORDER BY d.documentDate DESC')
    ...
    public function findCardHistorySales(Request $request,$id,$_locale="am"){
        ...
        if($customer) {
            if (null == $customer->getUser()) {
                ...
                $user_ = $customer->getUser();
                $docQuery = $documentRepository->createQueryBuilder('d')
                        ->select('d.amount,d.documentDate,b.branchTitle')
                        ->join('AppBundle:Account', 'acc', 'WITH', 'd.accountFrom = acc')
                        ->join('AppBundle:Account', 'a', 'WITH', 'd.accountTo = a')
                        ->join('AppBundle:User', 'u', 'WITH', 'acc.user = u')
                        ->join('AppBundle:Branch', 'b', 'WITH', 'a.branch = b')
                        ->join('AppBundle:TransactionType', 't', 'WITH', 'd.transactionType = t')
                        ->where('u =:user')
                        ->andWhere('t.id =:typeId')
                        ->setParameter('user', $user_)
            ...
            if (null == $customer->getUser()) {
                ...
                $docs = $em->createQuery(
                    'SELECT d.amount, d.bonusPoints,d.documentDate, b.branchTitle
                            ...
                            WHERE u=:user_ AND (t.id =:typeId AND d.documentDate >=:minDate AND d.documentDate <=:maxDate )
                            ORDER BY d.documentDate DESC')
                    ->setParameter('user_', $user_)
                    ->setParameter('typeId', 9)
                    ->setParameter('minDate', $minDate)
